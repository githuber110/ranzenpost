...
class CountingLetterService(ModuleService):
    def __init__(self, store, letters):
...
def test_a_new_letter_gets_the_plain_push_from_one_list_read():
    store = FakeStore()
    sent = []
    notifiers = {"letters": lambda n, m: sent.append(m)}
    Poller(CountingLetterService(store, {"letters": [_letter("a")]}), store=store, notifiers=notifiers).poll_once()
    second = CountingLetterService(store, {"letters": [_letter("a"), _letter("b")]})
    Poller(second, store=store, notifiers=notifiers).poll_once()
    assert sent == [messages.text_count("de", "notify.letters.new", 1)]
    assert second.letter_reads == 1
