...
for (const viewport of VIEWPORTS) {
  for (const lang of LANGUAGES) {
    ...
    test.describe(`tab bar and Post segment ${label}`, () => {
...
test.describe("the Post tab holds both areas", () => {
  test.use({ viewport: { width: 390, height: 844 }, locale: "de-DE" });

  test("inbox and archive stand side by side and one tap switches", async ({ page }) => {
    await goto(page);
    await page.locator(".tabbar .tab").nth(POST_TAB).click();
    await settled(page);
  ...
  test("the post tab comes back on the letters segment", async ({ page }) => {
    await goto(page);
    await page.locator(".tabbar .tab").nth(POST_TAB).click();
    await settled(page);
    await page.locator(".list-head .segment button").nth(1).click();
  ...
  test("a noticeboard row on the overview opens in place and comes back to the overview", async ({ page }) => {
    await goto(page);
    await settled(page);
    await page.locator('.panel[data-area="noticeboard"] [data-block] button, .panel[data-area="noticeboard"] button[data-block]').first().click();
    await settled(page);
    ...
    expect(await page.evaluate(() => state.view)).toBe("overview");
  });
});

test.describe("the chat chapter on the overview", () => {
  test.use({ viewport: { width: 390, height: 844 }, locale: "de-DE" });

  test("an unread room shows up on the overview and leads straight into the room", async ({ page }) => {
    await goto(page);
    await settled(page);
    const panel = page.locator('.panel[data-area="chat"]').first();
